...
class DailyTask(BaseEfTask):
    ...
    def exchange_the_outpost_goods(self, outpost_name):
        ...
        while attempt < max_attempts and not not_enough:
            ...
            for good in goods:
                ...
                self.log_info(f"选择货物进行兑换: {good.name}")
                ...
                self.log_info(f"{good.name} 当前数量: {num}")
                if num < 1000:
                    self.log_info(f"{good.name} 数量不足 (<1000)，停止兑换")
                    not_enough = True
                    break
    ...
    def transfer_to_home_point(self):
        ...
        if not self.wait_ocr(
            match="舰桥", box=screen_position.LEFT.value, time_out=40, log=True
        ):
